/**
 * `GET /api/system/skill/inspector`
 ...
 *     to every inspector agent's instructions.
 *
 * Read-only. Does not vary by agent. the body is a build-time
 * artifact (`packages/agents/src/inspector/system-prompt.md`) and
 * is identical across every inspector agent in the install. The
 * version string (`INSPECTOR_SYSTEM_PROMPT_VERSION`) drives BuiltAgent
 * cache invalidation; bumping it forces every cached agent to rebuild
 * on next access.
 *
