...
const baseFields = {
  ...
  /**
   * Per-agent token cap on the codebase inspection report payload each inspector wrapper
   * returns to the LLM. `null` means "use the module default"
   * (`CODEBASE_INSPECTION_REPORT_TOKEN_CAP`, 12_000 at the time of writing); set a
   ...
   * on `inspector.report.built`). Range is 2_000–64_000: floor leaves
   * room for graph + relationships after files, ceiling keeps a single
   * call from dominating the model's context window.
   */
  codebaseInspectionReportTokenCap: z.number().int().min(2_000).max(64_000).nullable(),
} as const
...
export const agentCreateInputSchema = z
  .object({
    ...
    /** Defaults to null on the server (= module default CODEBASE_INSPECTION_REPORT_TOKEN_CAP). */
    codebaseInspectionReportTokenCap: baseFields.codebaseInspectionReportTokenCap.optional(),
  })
  .strict()
...
export const agentResponseSchema = z.object({
  ...
  codebaseInspectionReportTokenCap: z.number().int().nullable(),
