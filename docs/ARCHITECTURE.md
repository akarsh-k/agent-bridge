...
4. **Two arms in parallel**:
   - Vector: `ORDER BY embedding <=> $1::vector ASC LIMIT $RETRIEVAL_DEPTH` (50) with `WHERE embedding_model = $fingerprint AND embedding IS NOT NULL` (excludes hierarchical parents). Depth was raised from 20 → 50 after the scorecard showed gold passages sitting at vector rank 17-50; the deeper pool lifts the recall ceiling from ~95% to ~98%.
   - BM25: `ORDER BY ts_rank_cd(tsv, q) DESC LIMIT $RETRIEVAL_DEPTH`, same WHERE filters. The tsquery is `plainto_tsquery` rewritten to OR-semantics (`' & '` → `' | '`) so partial term matches score like real BM25 instead of requiring every word.
...
7. **Rerank pool** (`buildRerankPool`) — top `RERANK_CANDIDATE_CAP` (40) fused candidates, plus up to `RERANK_BM25_RESCUE_SLOTS` (2) of the keyword arm's top hits appended when the down-weighted fusion left them below the cut (exact-identifier rescue). A wide cap is needed because gold passages sat as deep as vector rank ~40 on the scorecard.
8. **LLM-as-judge rerank (pointwise, batched)** — the model SCORES each candidate 0-10 for relevance; candidates are then sorted by `(score desc, RRF score desc)`. Two design points, both forced by the local judge:
   - **Pointwise, not listwise.** Replaced an earlier "return the best-first order" prompt that demoted gold the vector arm already had in the top-5 (rerank fell BELOW plain RRF). Because each passage is scored on its own 0-10 merit, an unparseable/empty reply or an unrated candidate just falls to score 0 → RRF order, so rerank can never rank below the fusion baseline.
   - **Batched (`RERANK_BATCH_SIZE` = 20).** Scoring all 40 candidates in one prompt dropped accuracy as the model lost focus; the pool is split into focused batches scored in parallel and merged (sound because scores are per-candidate, not relative). This let the cap stay wide (recall) without the focus penalty — end-to-end hit-rate went 91.5% → 93.2%. A finer 0-10 scale (vs an earlier 0-3) and `temperature: 0` decode further stabilized scoring. Skipped when ≤3 candidates.
   - **Chain-of-thought.** The prompt asks the judge to reason briefly about which candidate actually answers the query (looking past wording — synonyms/paraphrase) BEFORE emitting scores after a `SCORES:` marker (parsing keys off the last marker so the reasoning can't pollute the numbers). Forbidding reasoning ("scores only") had capped a small judge on vocabulary-mismatch answers; letting it think took hit-rate 93.2% → **96.6%** (MRR 0.68 → 0.88). Costs more output tokens per call.
